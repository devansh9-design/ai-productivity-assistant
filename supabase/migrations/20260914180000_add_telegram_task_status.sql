...
begin
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'task_id', pb.task_id,
      'status', case when t.id is null then null else t.status end,
      'status_reason', case when t.id is null then null else t.status_reason end,
  ...
  left join public.tasks t on t.id = pb.task_id and t.user_id = v_user_id
